...
func _on_Start_btn_pressed():
	...
	get_tree().get_root().remove_child(get_tree().get_root().find_child("SurvivalSetup", true, false))
...
func _on_PowerupHelpTextureRect_mouse_entered():
	var c = preload("res://scenes/instances/Popup_Help.tscn").instantiate()
...
func _on_PowerupHelpTextureRect_mouse_exited():
	get_tree().get_root().remove_child(get_tree().get_root().find_child("PopupHelp", true, false))
